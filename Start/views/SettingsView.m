...
- (id)initWithFrame:(CGRect)frame {
  ...
  if (self) {
    ...
    tackLabel.frame = (CGRect){{tackLogo.frame.origin.x - (tackSize.width + 8), (tackLogo.frame.origin.y+tackLogo.frame.size.height/2) - tackSize.height/2 + 3}, tackSize};
...
- (void)layoutSubviews {
  ...
  CGRect copyTextRect = CGRectMake(15, 20, copyTextSize.width, copyTextSize.height);
  CGRect versionTextRect = CGRectMake(frameSize.width - versionTextSize.width - 19, floorf(frameSize.height - (versionTextSize.height + 14)), versionTextSize.width, versionTextSize.height);
  
  CGRect underlineRect = CGRectMake(copyTextRect.origin.x, copyTextRect.origin.y + copyTextRect.size.height + 4, frameSize.width-(copyTextRect.origin.y*2), 1);
  CGRect introRect= CGRectMake(underlineRect.origin.x, underlineRect.origin.y + 50, introSize.width, introSize.height);
  CGRect scrollRect = CGRectMake(frameSize.width-180, 0, 180, frameSize.height);
  ...
  [timePicker setContentInset:
   UIEdgeInsetsMake(copyTextRect.origin.y - (optionHeight - copyTextSize.height)/2, 0, scrollRect.size.height-(copyTextRect.origin.y + copyTextRect.size.height) - (optionHeight - copyTextSize.height)/2, 0)];
...
- (void)animateTimePicker {
  if (!pickingSnooze) {
    // Save snooze time
    [[NSUserDefaults standardUserDefaults] setValue:[snoozeOptions objectAtIndex:selectedIndex] forKey:StartUserDefaultKey.snoozeTime];
