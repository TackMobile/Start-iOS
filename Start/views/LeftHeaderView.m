...
- (void)updateWithContentOffset:(float)cOffset {
  if (cellRect.origin.y+_sectionHeight  <= cOffset) {
    // Scrolled past it
    self.frame = CGRectMake(0, cellRect.origin.y+_sectionHeight, self.frame.size.width, self.frame.size.height);
  } else if (cOffset >= cellRect.origin.y + _topPadding && cellRect.origin.y+_sectionHeight > cOffset) {
    // Make it sticky
...
- (void)layoutSubviews {
  ...
  CGRect iconRect = CGRectMake((self.frame.size.width-icon.frame.size.width)/2, _topPadding, icon.frame.size.width, icon.frame.size.height);
